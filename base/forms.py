...
class SizeRestrictForm(): 
    # forderId에 들어갔을때 CustomUser에 storage 만큼만 담아야 함.
    # 첫번째 파라미터 : 저장 용량 / 두번째 파라미터 : 추가된 파일총 합.
    def storageOfSize_is_vaild(storage, addedVolume):
        storage = float(storage)
        print(f"Storage : {storage} / addedVolume : {addedVolume}")
        [convertedByte, unit] = addedVolume.split(" ")
        # byte 값으로 변환하기위한 logic
        if unit == "Byte":
            convertedByte = float(convertedByte)
        elif unit == "KB":
            convertedByte = float(convertedByte) * 1024
        elif unit == "MB":
            convertedByte = float(convertedByte) * 1024 * 1024
        elif unit == "GB":
            convertedByte = float(convertedByte) * 1024 * 1024 * 1024
        
        # byte로 변환시켜서 전체 용량에서 빼기
        
        if(storage < convertedByte):
            return False
        
        return True
        
    # 용량 측정 함수
    def helpByteCalcurator(Volume):
        if Volume < 1024:  # 1024 보다 작으면 GB 가 될 수 없음.
            return f"{Volume}Byte"
        elif Volume < 1024**2:
            return f"{Volume / 1024:.2f} KB"
        elif Volume < 1024**3:
            return f"{Volume / 1024**2:.2f} MB"
        else:
            return f"{Volume / 1024**3:.2f} GB"
