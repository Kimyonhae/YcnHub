    ...
    editbutton.addEventListener("click", (event) => {
        ...
        custom_input.addEventListener("keydown",(event) => {
            if(custom_input.value !== ""){
                if(event.key == 'Enter'){
                    fetchDataSubmit(`http://localhost:8000/${editbutton.id}/edit/`,custom_input.value,"PATCH");
                }
            }
        });
...
delete_button_list.forEach(delete_button => {
    delete_button.addEventListener("click",(event) => {
        ...
        if(qs){
            fetchDataSubmit(`http://localhost:8000/${delete_button.id}/delete_folder`,"","DELETE");
        }
    });
});
